...
function isPalindrome(str) {
  const newStr = str.toUpperCase().replace(/[^a-z0-9]/gi, '');
...
function findLongestWord(sentence) {
  const masWord = sentence.split(' ');
  return masWord.reduce((longestW, current) => {
    return current.length > longestW.length ? current : longestW;
  });
}
